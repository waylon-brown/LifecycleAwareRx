 ...
 * Fragment) is destroyed. To do this check out {@link #notDestroyed(LifecycleOwner)}.
 * 
 ...
 * takeLast(1) before calling this to only have the latest item emitted as soon as the LifecycleOwner is active. To do 
 * this check out {@link #subscribeWhenReady(LifecycleOwner, Observer)}.
 * 
 * An example of a stream taking advantage of both sides of the lifecycle binding:
 * 
 * getMyObservable()
...
public class LifecycleBinder {
     ...
     * LifecycleOwner is destroyed and the next item is emitted. This is preferred for 
     * Observables so that your stream can stop doing work rather than filter out each of the future items of the 
     * stream. This calls onComplete() unlike filter() so be sure to not access any views in onComplete() if using 
     * takeWhile().
     * 
     ...
     * accessing your views when they are ready, instead use one of the subscribeWhenReady() methods.
     * 
     ...
     */
    public static <T> SingleTransformer<T, T> subscribeWhenReady(LifecycleOwner lifecycleOwner,
                                                                 SingleObserver<T> singleObserver) {
        return new LifecycleTransformer<>(lifecycleOwner, new SingleWithObserver<>(singleObserver));
    }

    public static <T> ObservableTransformer<T, T> subscribeWhenReady(LifecycleOwner lifecycleOwner,
                                                                     Observer<T> observer) {
        return new LifecycleTransformer<>(lifecycleOwner, new ObservableWithObserver<>(observer));
    }

    public static <T> MaybeTransformer<T, T> subscribeWhenReady(LifecycleOwner lifecycleOwner,
